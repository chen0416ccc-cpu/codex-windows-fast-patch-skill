...
When `notifications\CURRENT_VERSION_READY.txt` appears, tell the user the guard has prepared a current-version protection package for the stuck/downloading state. When `notifications\READY.txt` appears, tell the user an installed-change staging package is ready. In both cases, the user must close Codex Desktop and run the generated `apply-command.ps1.txt` command from an external executor. Do not run apply automatically from this skill flow. The guard must not disable Microsoft Store updates, stop Desktop automatically, install patches automatically, set global `CODEX_HOME`, or save secrets, auth files, tokens, MCP credentials, `remote.json` contents, or browser profiles.
