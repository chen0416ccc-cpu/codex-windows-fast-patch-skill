...
- `scripts\watch-codex-desktop.ps1` creates or compares the baseline under `$env:USERPROFILE\.codex-fast-patch`, records events, writes file notifications, and triggers prepare after installed package/resource changes. It reads only package metadata, file hashes, local copied CLI version/hash, Desktop `config.toml` hash/length/timestamp, Codex-related WindowsApps directory names, recent Codex AppX deployment event excerpts, Codex-related BITS job metadata, and `codex doctor --json` update diagnosis. Config-only hash/length changes write `CONFIG_CHANGE.txt` and must not start native replacement builds. Download/deployment/runtime-update-only activity should write `UPDATE_ACTIVITY.txt` and trigger `prepare-patched-update.ps1`. `waiting_for_update_payload` and ordinary `needs_update_source` states may be retried by later scheduled runs, but `store_offline_authorization_required` / `0x8A150076` is an authorization boundary and should wait for sidecar/export/import payload instead of repeating the same Store download while `incoming` has no payload. The scheduled task must never apply.
- `scripts\check-codex-desktop-guard.ps1` is the read-only status panel. Use it before explaining whether the guard is idle, waiting, ready, failed, or safe. It reads the scheduled task, state root markers, latest staging manifest, incoming payload count, Desktop process count, and bounded `watch.log` tail. It must not start Sandbox, run winget, stop Desktop, or apply patches. The normal current stuck-update state after Store offline auth failure is `waiting_for_external_payload` with reason `store_offline_authorization_required`.
- `scripts\start-codex-desktop-guard-ui.ps1` and `scripts\stop-codex-desktop-guard-ui.ps1` provide a local read-only browser UI for non-programmer status checks. The UI binds to `127.0.0.1`, reads `check-codex-desktop-guard.ps1 -Json`, and must remain status-only: no Sandbox start, no winget, no Desktop stop, no apply.
- `scripts\acquire-codex-update-package.ps1` is the active update-source acquisition path. It defaults to Microsoft Store ID `9PLM9XGG6VKS` with `winget download`, writes under `$env:USERPROFILE\.codex-fast-patch\downloads`, unpacks candidate MSIX/MSIXBundle files, and never installs or stops Desktop. If Store offline download requires Microsoft Entra ID authorization, proxy, network, or a manual package source, it writes `NEEDS_UPDATE_SOURCE`; for Microsoft Entra ID authorization, switch to the clean Windows / VM official-install export/import sidecar-source flow.
- `scripts\export-installed-codex-payload.ps1` and `scripts\import-codex-update-payload.ps1` are the sidecar source workflow for the chicken-and-egg case where the current patched Desktop is stuck downloading and this machine cannot acquire the Store offline package. Export runs on a clean Windows / VM / another user environment with official Codex installed and writes a sidecar zip containing only the package layout plus `codex-payload-manifest.json` and `hashes.json`. Import runs on the patched machine, puts the payload under `$env:USERPROFILE\.codex-fast-patch\incoming`, and by default triggers `prepare-patched-update.ps1 -PayloadRoot <incomingPayloadRoot>`.
- `scripts\start-sidecar-acquire.ps1`, `scripts\check-sidecar-acquire.ps1`, and `scripts\stop-sidecar-acquire.ps1` are the Windows Sandbox sidecar acquisition control plane. The Codex Desktop main session is only the dispatcher: start launches Sandbox and returns immediately, the Sandbox worker writes `$env:USERPROFILE\.codex-fast-patch\sidecar-output\status.json`, check is a short read-only query that tails at most 60 log lines and classifies `READY` / `FAILED` / `NEEDS_ACTION` / `RUNNING`, and stop stops only Sandbox. Do not run long `while` / `for` / `Start-Sleep` polling loops from the Desktop session while waiting for Sandbox, winget, Store, Add-AppxPackage, DISM, or WindowsApps scans.
- The Sandbox sidecar uses `winget download`, not `winget install`. If `check-sidecar-acquire.ps1` reports `store_offline_authorization_required` or `0x8A150076`, treat it as Microsoft Store offline distribution requiring Microsoft Entra ID authorization; do not keep debugging host DNS. Switch to the clean Windows / VM official-install export/import sidecar-source flow.
- `scripts\discover-codex-update-payload.ps1` looks for a usable new update payload: an unpacked `OpenAI.Codex_*` directory that differs from the current live package/content, a sidecar/imported payload under `$env:USERPROFILE\.codex-fast-patch\incoming`, or a package unpacked by `acquire-codex-update-package.ps1`. The payload must contain `app\resources\codex.exe`, `app\resources\app.asar`, and `app\Codex.exe`. If active acquisition cannot produce a payload, it writes `NEEDS_UPDATE_SOURCE`; if it is only waiting for Store deployment to expose one, it writes `WAITING_FOR_UPDATE_PAYLOAD`.
- `scripts\prepare-patched-update.ps1` is the V2 update takeover path. It discovers/acquires the update payload, validates the payload version/native mapping, builds the matching remote-control native replacement only when safe, runs the targeted MSIX/ASAR patch flow against the payload using `-SourceRoot`, and writes `PATCHED_UPDATE_READY` only when a patched MSIX exists. For patched-update payloads, a mapping may be inferred only from that payload's own `app\resources\codex.exe --version` as `rust-vx.y.z` / `AppServerVersion x.y.z`; never reuse an old binary for a new payload. Unreadable versions, incomplete hashes, build failures, or hash mismatches must end in `NEEDS_ACTION` / `needs_manual_version_mapping`.
- `scripts\prepare-fast-patch.ps1` never stops or modifies the live Desktop install. `-Mode InstalledChange` builds a prepared native replacement only when the mapping is known safe. `-Mode UpdateActivity` can still snapshot the current live `resources\codex.exe` into staging as `CURRENT_VERSION_READY`, but this is a fallback protection artifact, not the patched-update path. V1 safe mapping is `OpenAI.Codex_26.623.9142.0` / `codex-cli 0.142.4` / `rust-v0.142.4` / `AppServerVersion 0.142.4`.
- `scripts\apply-prepared-fast-patch.ps1` is for external PowerShell or VS Code Codex only. By default it refuses Codex Desktop-launched processes and refuses to continue while Desktop is still running. It accepts `ready`, `current_version_ready`, and `patched_update_ready` manifests. `current_version_ready` must still match the installed package version and package full name; `patched_update_ready` installs only an already prepared signed MSIX. It must not be called by the scheduled task.
...
# Current machine Windows Sandbox sidecar, short-command mode:
...
# Guard status panel, read-only:
powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\check-codex-desktop-guard.ps1"

# Guard local Web UI, read-only:
...
When `notifications\NEEDS_UPDATE_SOURCE.txt` appears, tell the user the guard did try to acquire the Store update package, but the machine did not provide a usable payload; report the last `winget download` command and offer the sidecar export/import path above. When `notifications\WAITING_FOR_UPDATE_PAYLOAD.txt` appears, tell the user the guard saw update/download activity but cannot yet see a usable new payload. When `notifications\PATCHED_UPDATE_READY.txt` appears, tell the user the new payload has been patched and packaged; they must close Codex Desktop and run the generated `apply-command.ps1.txt` command from an external executor. `CURRENT_VERSION_READY.txt` is only a fallback current-version protection package, while `READY.txt` is installed-change staging. Do not run apply automatically from this skill flow. The guard must not disable Microsoft Store updates, stop Desktop automatically, install patches automatically, set global `CODEX_HOME`, or save secrets, auth files, tokens, MCP credentials, `remote.json` contents, or browser profiles.
