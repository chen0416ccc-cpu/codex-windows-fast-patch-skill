...
The repair must preserve the installed package's `plugins\chrome\scripts\browser-client.mjs` bytes exactly. Older Node REPL builds expose privileged browser capabilities when the imported browser-client SHA-256 matches the trust list embedded in `app.asar`; current builds can instead provision a trusted browser service through `NODE_REPL_TRUSTED_SERVICES` and `browserServicePath`, with the packaged client enforcing that contract through `globalThis.nodeRepl.rpc`. Codex Desktop `26.814`-style builds may additionally use the complete native-host path contract and `browserClientPath` / `browserServicePath` state. If the stable cache resolves through a junction outside `CODEX_HOME`, the user-level and Desktop-generated `NODE_REPL_TRUSTED_CODE_PATHS` values must contain the resolved physical marketplace/cache roots, and the installed ASAR must contain `CODEX_NODE_REPL_TRUSTED_PATHS_V1`. Rewriting a `node:process` import or process shim breaks either contract. Normal repair restores the packaged bytes to both the stable marketplace and versioned cache. Strict verification requires both copies to match the packaged SHA-256 and accepts only a complete legacy ASAR-hash, packaged service-trust, or native-host path contract.
