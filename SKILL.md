...
- Current Codex builds can use a lightweight versioned Computer Use cache with no `latest` junction and no plugin-local `node_modules`; `@oai/sky` lives under `%LOCALAPPDATA%\OpenAI\Codex\runtimes\cua_node`. Treat that layout as healthy when the versioned cache matches the installed package, client import succeeds, and the helper returns `list_windows`. Only run `-VerifyOnly` when those checks find a missing or changed cache file or an unusable runtime.
...
- The Computer Use plugin cache matches the installed package. Support both legacy `computer-use\latest\node_modules\@oai\sky` and the current lightweight versioned cache plus `%LOCALAPPDATA%\OpenAI\Codex\runtimes\cua_node`; the helper transport can at least return `list_windows` (or legacy screen info).
