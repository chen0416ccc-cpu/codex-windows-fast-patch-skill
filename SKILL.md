...
- Use the CUA surface lock case when `cua.getState()` succeeds but native-app methods are missing (`cua.getApp is not a function`, `cua.listApps is not a function`, no `computer`/`getApp`/`listApps` in `Object.keys(cua)`) while `scripts\install-computer-use-local.ps1 -StrictVerifyOnly` passes and the `codex-computer-use-*` pipe exists. The materialized plugin cache `.mcp.json` pins `CUA_REPL_ENABLED_SURFACES` to `browser`, but editing that file is not a repair: the Desktop plugin reconcile rewrites it on every Desktop start, which is why the symptom keeps returning. Run `scripts\repair-cua-surface-lock.ps1 -VerifyOnly`, then `-Install`, start a fresh conversation, and require the three verification signals — see the surface lock case in `references/restriction-debug-cases.md`.
- Use the Windows app-binding case when the surface lock is already repaired, `Object.keys(cua)` contains `computer`/`getApp`/`listApps`, and a Computer Use test still fails because `cua.getApp` or `cua.listApps` rejects with `Native app bindings are unavailable for windows.` In `@oai/cua`, both methods throw unless the service reports `sky.target === "mac"`, so they are unusable on Windows by design. The working native API is the window-based `cua.computer.*` surface (`list_windows`, `activate_window`, `get_window_state`, `click`, `type_text`, ...), and the shipped `resources\computer-description.md` never mentions it — it documents only the macOS entry point, which is why the model keeps calling the broken one. Run the same `repair-cua-surface-lock.ps1 -Install`, which patches that description too, and verify with `scripts\probe-cua-surface.py` — see the Windows app-binding case in `references/restriction-debug-cases.md`. Do not report native control as unavailable on Windows; report it as available through the window API.
...
If the runtime responds, the `codex-computer-use-*` pipe exists, and `-StrictVerifyOnly` passes, but `Object.keys(cua)` still lists no `computer`, `getApp`, or `listApps`, the plugin surface list — not the runtime — is the problem. Check and repair it before anything heavier:
...
`-VerifyOnly` is a gate and fails while any target is unpatched; `-Json` and the default report never write; `-Rollback` restores from the adjacent `<file>.bak-*`. Two independent patches are applied to the versioned cache copy plus both marketplace source copies: the `computer` surface is forced inside `scripts\launch.mjs`, and `resources\computer-description.md` is rewritten so the injected `js` tool description stops offering the macOS-only `cua.getApp` to a Windows conversation. Editing `.mcp.json` alone is not durable: the Desktop plugin reconcile regenerates it on every Desktop start from a surface list that only includes `computer` when `platform` is `darwin`. `scripts\test-cua-surface-lock-patterns.ps1` covers both patches (anchor, newline preservation, idempotency, rollback, and the unknown-layout refusal) against throwaway fixtures, and `python "$SkillRoot\scripts\probe-cua-surface.py"` reads the live tool description, `Object.keys(cua)`, and `cua.computer.list_windows()` out of a real kernel as the independent acceptance step.
...
- For the CUA surface lock, `scripts\repair-cua-surface-lock.ps1 -VerifyOnly` reports the `surface` and `description` targets on every `unified-computer-use` copy as `patched` and exits clean, the versioned cache `scripts\launch.mjs` SHA-256 matches `4312E22A...419175` (original `A50B6687...66A2597`) and `resources\computer-description.md` matches `C72E08A0...FB9291` (original `46BD5B39...EC4514`), and `scripts\test-cua-surface-lock-patterns.ps1` passes every assertion. `scripts\probe-cua-surface.py` must report `windows guidance present`, list `computer`, `getApp`, and `listApps` in `Object.keys(cua)`, and print a `windows api` line of `windows/<n>` with `n > 0` while `CUA_REPL_ENABLED_SURFACES` is forced back to `browser`: that is the signal that proves the fix survives the next Desktop start, because a fresh `.mcp.json` alone would report it only until the reconcile runs.
- On Windows, native Computer Use is exercised through the window-based `cua.computer.*` API, and the injected `js` tool description says so. `cua.getApp` / `cua.listApps` throw `Native app bindings are unavailable for windows.` on every Windows build; that is an expected upstream boundary and not a defect to report as "Computer Use unavailable".
