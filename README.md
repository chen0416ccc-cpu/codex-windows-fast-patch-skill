...
- `scripts/test-cua-surface-lock-patterns.ps1`：`repair-cua-surface-lock.ps1` 的回归测试，全部使用一次性 fixture，覆盖两处补丁的 anchor 匹配、行尾保持、幂等、回滚以及「未知布局只报告不改写」。
- `scripts/probe-cua-surface.py`：独立验收探针。用插件自带 `.mcp.json` 的 env 启动 `cua_repl`，把 `CUA_REPL_ENABLED_SURFACES` 强制回 `browser`（Desktop 对账实际写入的值），然后从活内核读取注入的工具描述、`Object.keys(cua)` 与 `cua.computer.list_windows()`，用于证明修复能扛过下一次 Desktop 启动。
