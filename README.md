...
- 提供 Windows-only “小守卫 / Codex Desktop Guard”：发现 Codex Desktop 静默更新/下载活动后先寻找新版本 update payload，能拿到新 payload 时准备 patched update staging，拿不到时明确等待 payload，并提醒用户用外部执行器手动应用。
...
- `scripts/prepare-patched-update.ps1`：Codex Desktop Guard patched update 准备脚本；有新 payload 时才构建修复后的更新包，找不到 payload 时写等待状态。
...
- `notifications`：`UPDATE_ACTIVITY.txt`、`WAITING_FOR_UPDATE_PAYLOAD.txt`、`PATCHED_UPDATE_READY.txt`、`CURRENT_VERSION_READY.txt`、`CHANGE.txt`、`READY.txt`、`NEEDS_ACTION.txt` 等文件通知。
...
当发现下载/部署/运行时更新迹象，例如 Desktop 一直显示“正在下载”但包和资源还没变，watch 会写 `UPDATE_ACTIVITY.txt`，然后优先触发 `prepare-patched-update.ps1`。这条 V2 主线会先调用 `discover-codex-update-payload.ps1` 查找新版本 update payload，例如不同于当前 live package 的 `OpenAI.Codex_*` unpacked package。能找到新 payload 时，它才会对新 payload 重新跑 fast-patch / remote-control 修复，生成 `PATCHED_UPDATE_READY`。找不到 payload 时，它会写 `WAITING_FOR_UPDATE_PAYLOAD`，表示官方更新还没有落到小守卫可处理的位置；不会假装已经修好，也不会拿旧 `codex.exe` 硬套新版本。
