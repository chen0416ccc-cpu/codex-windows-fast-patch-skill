...
目前按整文件 SHA-256 支持服务版本 `26.924.22138`、`26.924.20706`、`26.917.71314`、`26.908.40834`、`26.908.70816`，不是所有同前缀版本。`26.917.71314` 已在 Windows 上完成无额外登录的真实导航、输入、点击及服务端 `x-browser-agent` 请求头验收。`26.924.22138` 已通过本地行为、防护、缓存范围及可信客户端契约检查，但这些检查不等同于真实浏览器验收；`26.924.20706` 的离线测试证据来自已审查的 PR #73。两个 `26.908` 档位只有离线行为与防护回归，未重装旧 Desktop 做实机验收。
