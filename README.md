...
如果只检测到 `$env:USERPROFILE\.codex\config.toml` hash/length 变化，watch 只写 `CONFIG_CHANGE.txt` 并更新 baseline；它不会读取配置正文，也不会为 config-only 事件启动 native replacement build。若 config-only 变化发生时仍有下载/更新活动，watch 会继续检查 patched-update source 状态。
...
prepare 不停止、不卸载、不重装、不修改 live Desktop install。installed-change 路径仍只内置已验证映射：`OpenAI.Codex_26.623.9142.0` / `codex-cli 0.142.4` / `rust-v0.142.4` / `AppServerVersion 0.142.4`。patched-update 路径如果能从新 payload 的 `app\resources\codex.exe --version` 安全读出 `codex-cli x.y.z`，会用 `rust-vx.y.z` / `AppServerVersion x.y.z` 构建匹配 replacement；这不是复用旧二进制。如果 native version 读取失败、payload hash 不完整或构建失败，会写 `NEEDS_ACTION` / `needs_manual_version_mapping`，不会盲猜。
