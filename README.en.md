...
To quickly answer "what is the guard doing now?", run the read-only status panel:
...
If only `$env:USERPROFILE\.codex\config.toml` hash/length changed, watch writes `CONFIG_CHANGE.txt` and updates the baseline; it does not read config contents and does not start a native replacement build for that config-only event. If config-only changes happen while download/update activity is still present, watch continues checking the patched-update source state.
