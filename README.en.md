...
- `scripts/test-cua-surface-lock-patterns.ps1`: Regression test for `repair-cua-surface-lock.ps1`, using throwaway fixtures only. Covers anchor matching, newline preservation, idempotency, rollback, and the refusal to edit an unrecognized layout for both patches.
- `scripts/probe-cua-surface.py`: Independent acceptance probe. It starts `cua_repl` with the plugin's own `.mcp.json` environment while forcing `CUA_REPL_ENABLED_SURFACES` back to `browser` (the value the Desktop reconcile writes), then reads the injected tool description, `Object.keys(cua)`, and `cua.computer.list_windows()` out of a live kernel, which is what proves the repair survives the next Desktop start.
